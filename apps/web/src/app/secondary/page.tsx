...
export default function SecondaryMarketPage() {
  ...
  const [sortBy, setSortBy]     = useState<"newest" | "ask_asc" | "ask_desc" | "premium_asc" | "premium_desc">("newest");
  ...
  const visible = listings
    .filter((l) => filter === "all" || l.startup.category === filter)
    .filter((l) => !search || l.startup.name.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
      switch (sortBy) {
        case "ask_asc":      return Number(a.askPriceINR) - Number(b.askPriceINR);
        case "ask_desc":     return Number(b.askPriceINR) - Number(a.askPriceINR);
        case "premium_asc": {
          const pa = Number(a.askPriceINR) - Number(a.algorithmicPriceINR);
          const pb = Number(b.askPriceINR) - Number(b.algorithmicPriceINR);
          return pa - pb;
        }
        case "premium_desc": {
          const pa = Number(a.askPriceINR) - Number(a.algorithmicPriceINR);
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="mb-4 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/[0.03] pl-8 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:border-green-500/30 focus:outline-none"
            />
          </div>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as typeof sortBy)}
            className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-gray-400 focus:border-green-500/30 focus:outline-none"
          >
            <option value="newest">Newest first</option>
            <option value="ask_asc">Ask price: low → high</option>
            <option value="ask_desc">Ask price: high → low</option>
            <option value="premium_asc">Premium: low → high</option>
            <option value="premium_desc">Premium: high → low</option>
          </select>
          {(search || sortBy !== "newest") && (
            <button onClick={() => { setSearch(""); setSortBy("newest"); }} className="text-xs text-gray-600 hover:text-gray-400 transition">
              Clear
            </button>
          )}
        </div>
